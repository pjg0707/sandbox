...
def create_expynses_icon(folder: Path, icon_size=256):# Colors
    ...
    draw.rectangle([0, tab_height, icon_size, folder_height + tab_height], fill=folder_color)
    ...
    text_position = ((icon_size - text_width) // 2, (folder_height + tab_height - text_height) // 2)
...
def build_icon_library(ico_folder: Path, output_name: str = "custom_icon_library.dll", target_folder: Path = Path("C:/Windows")):
    ...
    try:
        ...
        print(e)
        print("Sugerencias:")
        print("- Asegurate de que el archivo flags.rc existe y tiene rutas ABSOLUTAS correctas.")
        print("- Ejecutá manualmente ese mismo comando en Git Bash para ver el mensaje de error exacto.\n")
        raise

    print("--- Compilando con gcc ---")
    subprocess.run(
        ["gcc", "-shared", "-o", dll_abs, res_abs],
        check=True
    )
